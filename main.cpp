...
Student generateRandomStudent(string index) {
    ...
    cout << "Student " << index << ": " << "school:" << school << ", age:" << age << " | math:" << math << " | russian:" << rus << " | ";
    vector<ExamResult> examResults;
    ...
    for (const string& subject : {"physics", "chemistry", "biology", "informatics"}) {
        if (rand() % 2 == 0) {
            int score = rand() % 101;
...
bool hasPerfectScore(const Student& student) {
    return any_of(student.examResults.begin(), student.examResults.end(), [](const ExamResult& result) {
...
void processMultiThreaded(const vector<Student>& students, int numThreads) {
            ...
            for (int j = startIndex; j < endIndex; ++j) {
                if (hasPerfectScore(students[j])) {
                    ...
                    if (schoolIt != schoolsMap.end()) {
                        ...
                        localMutex.lock();
                        schoolsMap[students[j].schoolNumber] = School{students[j].schoolNumber, 1, {students[j].name}};
                        localMutex.unlock();
                    }
                }
            }
    ...
    transform(schoolsMap.begin(), schoolsMap.end(), schools.begin(), [](const pair<int, School>& entry) {
        return entry.second;
    });
...
int main() {
    ...
    cout << fixed << setprecision(6) << "Single-threaded time: " << elapsedSingleThreaded.count() << " seconds\n";
    cout << fixed << setprecision(6) << "Multi-threaded time: " << elapsedMultiThreaded.count() << " seconds\n";
